...
@synthesize mapView, locationManager, tableView, favorite, name;
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [super viewDidAppear:true];
        pins = false;
    [self.mapView setRegion:MKCoordinateRegionMake(currentLocation.coordinate, MKCoordinateSpanMake(0.1f, 0.1f)) animated:NO];
...
-(void) queryGooglePlacesDouble: (NSString *) restName {
    ...
    NSString * restNa = [restNam stringByReplacingOccurrencesOfString:@"-" withString:@"%20"];
    NSString * restN = [restNa stringByReplacingOccurrencesOfString:@"&" withString:@""];
    NSString *url = [NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/search/json?location=%f,%f&radius=%@&rankBy=distance&sensor=true&name=%@&key=%@", currentLocation.coordinate.latitude, currentLocation.coordinate.longitude, [NSString stringWithFormat:@"%i", 20000], restN, kGOOGLE_API_KEY];
    NSLog(@"looking for %@", restN);
    NSURL *googleRequestURL=[NSURL URLWithString:url];
...
-(void)plotPositions:(NSArray *)data {
    ...
    for (int i=0; i<num; i++) {
        ...
        NSString *rName=[place objectForKey:@"name"];
        ...
        MapPoint *placeObject = [[MapPoint alloc] initWithName:rName address:vicinity coordinate:placeCoord];
